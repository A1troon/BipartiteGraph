...
class GFG{
    ...
    {   GraphGenerator.getInstance().newRandomGraph(10).printGraph();
        Kuhn.start();
        BipGraph g = new BipGraph(GraphGenerator.getInstance().getL(), GraphGenerator.getInstance().getN()-GraphGenerator.getInstance().getL());
        List<Integer>[] lists=GraphGenerator.getInstance().addOneToListEdge();
        for (int i=1;i<lists.length;i++)
            for (int j:lists[i]) {
                g.addEdge(i, j);
                System.out.println(i+" "+(int)(j+GraphGenerator.getInstance().getL()));
            }
